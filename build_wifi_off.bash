...
# User varibales
package_identifier="com.yourdomain.wifi-off"
package_version="1.0"
package_output_name="wifi-off.pkg"

# Intenral variables
exit_status=0
parent_directory="`dirname \"${0}\"`" ; if [ "`echo "${parent_directory}" | grep -e "^/"`" == "" ] ; then parent_directory="`pwd`/${parent_directory}" ; fi
temporary_build_directory=`mktemp -d /tmp/build_directory.XXXXXXXXXXXXX`
...
# clean up files found int the temporary root build directory
rm -f "./root/.DS_Store"
rm -f "./root/.gitignore"
